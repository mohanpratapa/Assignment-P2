...
QUnit.test('TEST add', assert => {
  assert.equal(multiplication(1), 72, 'Positive integers')
  assert.equal(multiplication(-1), -72, 'Negative integers')
  assert.equal(multiplication(2), 144, 'Mixed')
  assert.equal(multiplication(-2), -144, 'Mixed')
  assert.equal(multiplication(3), 216, 'Mixed')
})
